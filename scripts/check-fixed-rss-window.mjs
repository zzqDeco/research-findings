...
async function loadSource(sourceId) {
  ...
  if (!source) {
    throw new Error(`No enabled AI RSS source is configured for ${sourceId}`)
  }
  return source
}
...
async function main() {
  ...
  const response = await fetch(feedUrl, {
    headers: {
      ...
      pragma: 'no-cache',
      'user-agent': 'research-findings-fixed-rss-scan/1.0',
    },
  ...
  const inspection = inspectRssWindow(feed, { start, end })
  const result = {
    sourceId: source.id,
